...
uses SysUtils, Windows, Classes, UnitVec, UnitLightmap;
...
procedure SaveToHDRFile(const FileName: String; const Width, Height: Integer;
  const Lightmaps: PLightmapColor);
function LoadFromHDRFile(const FileName: String; const Width, Height: Integer;
  const Lightmaps: PLightmapColor): eLoadStateHDR;
  ...
  if ((Width < 8) or (Width > $7FFF)) then
    begin
      ...
      if ((Width > 0) and (Height > 0)) then
        begin
          SetLength(Buffer, LightmapColorSize*SqrSize);
          ...
          BlockWrite(FileHDR, Pointer(@Buffer[0])^, LightmapColorSize*SqrSize);
        end;
    end
  else
    begin
      ...
      SetLength(Buffer, LightmapColorSize*Width);
  ...
  if ((Width < 8) or (Width > $7fff)) then
    begin
      ...
      if ((Width > 0) and (Height > 0)) then
        begin
          SetLength(Buffer, LightmapColorSize*SqrSize);
          BlockRead(FileHDR, Pointer(@Buffer[0])^, LightmapColorSize*SqrSize);
          // Correct Lightmap RGBExp to Radiance RGBE
          for i:=0 to (SqrSize - 1) do
            begin
          ...
          begin
            ...
            if ((RunLenHeader[0] <> 2)
              ...
              begin
                ...
                SetLength(Buffer, LightmapColorSize*(SqrSize - 1));
                BlockRead(FileHDR, Pointer(@Buffer[0])^, LightmapColorSize*(SqrSize - 1));
                // Correct Lightmap RGBExp to Radiance RGBE
